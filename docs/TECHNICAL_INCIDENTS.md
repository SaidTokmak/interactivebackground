...
## FEATURE-009 — Çoklu widget kataloğu ve kalıcı Pomodoro

- Tarih: 15 Temmuz 2026
- Durum: Uygulandı; görsel, migration ve release düzeyinde doğrulandı
- Final rapora dahil et: Evet

Tek görev alanına özel `widget_layouts` modeli `desktop_widgets` kataloğuna
genişletildi. Her kayıt monitör, tür, normalize konum/boyut, kilit, grid,
görünürlük ve sıralama bilgisini taşır. Kullanıcı Odak, Kanban, Pomodoro, Saat
ve Tarih öğelerini ekleyebilir; her öğeyi bağımsız sürükleyebilir,
boyutlandırabilir, gizleyebilir, kilitleyebilir, çoğaltabilir, sıralayabilir veya
kaldırabilir. Monitör başına 12 öğe sınırı hem arayüz hem Rust katmanında
uygulanır.

Eski yerleşimler silinmedi. `app_meta.desktop_widgets_v1` işareti yoksa legacy
satırlar yeni tabloya monitör ve şablon ayrımı korunarak taşınır; eski kayıt
yoksa mevcut hedef monitör ve şablondan ilk görev widget'ı oluşturulur. İşaret
en son yazıldığı için yarım kalan migration güvenle yeniden denenebilir, başarılı
migration ise kullanıcı bütün öğeleri sonradan silse bile tekrar çalışmaz.

Her widget türü kendi minimum ve maksimum alan kurallarına sahiptir. Mevcut
normalize sürükleme motoru öğe türünü dikkate alacak şekilde genelleştirildi;
sekiz yönlü boyutlandırma, yüzde 1,5 kenar payı, grid, kenara yapışma ve pointer
capture bütün katalog öğelerinde ortak çalışır. Sıralama değeri hem yönetim
listesini hem wallpaper üzerindeki katman sırasını belirler.

Pomodoro durumu `pomodoro_states` tablosunda çalışma/mola süreleri, aktif mod,
kalan saniye ve kesin Unix bitiş zamanı olarak tutulur. Sayaç çalışırken pencere
kapanırsa yeniden açılışta kalan süre sistem saatinden hesaplanır; süre geçmişse
native katman bir sonraki moda geçip sayacı kontrollü biçimde durdurur. Çoğaltma
özel süreleri kopyalar ancak ikinci sayacı güvenli biçimde durdurulmuş başlatır.

Başlatma anında resmi Tauri notification eklentisi Windows'a kesin bitiş anı
için zamanlanmış bildirim bırakır. Manuel duraklatma, sıfırlama veya atlama eski
bildirimi iptal eder; doğal süre dolumu bildirimi iptal etmeden Rust durumunu
normalize eder. Bildirim metni seçili Türkçe/İngilizce dile göre çalışma ve mola
sonu için ayrı üretilir.

Tarayıcı üretim görünümü 1440×1200 çözünürlükte kontrol edildi; katalog mevcut
tasarım sistemi içinde yatay taşma oluşturmadan yerleşti. Rust testleri katalog
izolasyonu, ekleme, çoğaltma, sıralama, silme, legacy migration'ın tek sefer
çalışması, Pomodoro yapılandırması, zaman aşımı ve çoğaltılan sayaç ayarlarını
kapsar. Frontend TypeScript üretim derlemesi beş widget türünü ve iki dildeki
kontrolleri doğrular.

Release binary gerçek kullanıcı veritabanında `--hidden` çalıştırıldı. On bir
sütunlu `desktop_widgets`, `pomodoro_states` ve `app_meta` tabloları oluştu;
mevcut Kanban tercihi hedef `DISPLAY1` için ilk görünür widget'a dönüştürüldü.
Migration işareti `done` olarak yazılırken mevcut beş görev ve tüm uygulama
